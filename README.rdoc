...
A rewrite of the wonderful dm-is-remixable[http://github.com/datamapper/dm-more/dm-is-remixable] library by Cory O'Daniel.
...
== API ideas (not cut into stone at all)

  module Addressable
    # This will use extlib/lazy_module to construct the remixable module
    remixable :as => :addressable do # :as will default to :addressable
      property :id,     Serial
  ...
  module Linkable
    # This will use extlib/lazy_module to construct the remixable module
    remixable :as => :linkable do # :as will default to :linkable
      property :id,  Serial
      property :uri, URI
    end
  end
  ...
  #
  def remix(cardinality, relationship_name, options)
    # TODO implement
  end
